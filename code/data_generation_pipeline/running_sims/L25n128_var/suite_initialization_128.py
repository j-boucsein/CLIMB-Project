...
######################## create random seeds for music ###########################

random_seeds = [random.randrange(1, 10**5) for i in range(len(cosmo_parameters))]
...
for counter, (Omega_m, Omega_b, Omega_L, hubble_par) in enumerate(cosmo_parameters):
    ...
    random_seed_this_gp = random_seeds[counter]
    print(f"random_seed: {random_seed_this_gp:05}")
    ...
    with open(path_to_param_music_file, "r") as file:
        for row in file:
            ...
            if "seed[7]" in row:
                row = f"seed[7]           = {random_seed_this_gp:05}\n"
            file_content.append(row)
        ...
        for row in file:
            ...
            if "H0" in row:
                if not row.split()[-1] == f"{100*hubble_par:.2f}":
                    ...
                    error = True
            if "seed[7]" in row:
                if not row.split()[-1] == f"{random_seed_this_gp:05}":
                    print(f"Error! -> seed[7] not correctly set in {path_to_param_music_file}")
                    error = True
    ...
    with open(path_to_slurm_script, "r", encoding="utf-8") as file:
        for row in file:
            if "SBATCH -J" in row:
                row = f"#SBATCH -J CLIMB-GP-{counter}-var\n"
            file_content.append(row)
