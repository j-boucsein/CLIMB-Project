...
number_of_cpus=(32 72 144 216 288) # number of cpus to use
...
for n_cpus in "${number_of_cpus[@]}"; do
    ...
    SIM_DIR="$BASE_SIM_DIR/sim_n${n_cpus}_cpus"
    mkdir -p "$SIM_DIR"
    ...
    sed -i "s/^levelmin *= *.*/levelmin           = 7/" "$SIM_DIR/param_music.txt"
    sed -i "s/^levelmin_TF *= *.*/levelmin_TF        = 7/" "$SIM_DIR/param_music.txt"
    sed -i "s/^levelmax *= *.*/levelmax           = 7/" "$SIM_DIR/param_music.txt"
    ...
    echo "[$n_cpus cpus] $SBATCH_OUTPUT" >> "$BASE_SIM_DIR/sbatch_n_cpus_log.txt"
